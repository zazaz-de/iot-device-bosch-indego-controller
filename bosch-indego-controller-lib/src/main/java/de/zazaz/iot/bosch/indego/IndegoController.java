...
/**
 * Instances of this class handle the communcation the the Indego device and
 * provices methods to control and query the device.
 */
public class IndegoController {
	...
	/**
	 * for limiting the amount of status query requests on the server a minimum
	...
	/**
	 * the respons, which was sent by the server after a successfull
	 * authentication (contains session, device serial, etc.)
	 */
	 ...
	 */
	public IndegoController(final String baseUrl_, final String username_, final String password_) {
	 ...
	 *            the password for authenticating
	 */
	public IndegoController(final String username_, final String password_) {
	...
	/**
	 * @param baseUrl_
	 *            the user specified base url
	 * @return the reformatted and normalized base url
	 */
	private String normalizeBaseUrl(final String baseUrl_) {
	 ...
	 *             in case of wrong authentication informations
	...
	public void connect() throws IndegoAuthenticationException, IndegoException {
		...
		try {
			...
			lastStateQueryTs = 0;
		} catch (final IndegoException ex) {
			safeCloseClient();
			throw ex;
	...
	/**
	 * This queries the device state from the server or returns a cached state
	 * if the last query was lass than <code>MIN_STATE_QUERY_INTERVAL_MS</code>
	 * milliseconds ago.
	 * 
	...
	public DeviceStateInformation getState() throws IndegoException {
		synchronized (this) {
			if (deviceStateCache != null && System.currentTimeMillis()
					- MIN_STATE_QUERY_INTERVAL_MS < lastStateQueryTs) {
				return deviceStateCache;
			}
			final DeviceStateInformation state = doGetRequest(
					"alms/" + session.getAlmSn() + "/state", DeviceStateInformation.class);
	...
	public DeviceCalendar getCalendar() throws IndegoException {
		synchronized (this) {
			final DeviceCalendar calendar = doGetRequest("alms/" + session.getAlmSn() + "/calendar",
	...
	public LocationWeather getWeather() throws IndegoException {
		synchronized (this) {
			final LocationWeather weather = doGetRequest(
					"alms/" + session.getAlmSn() + "/predictive/weather", LocationWeather.class);
	...
	public int getPredictiveAdjustment() throws IndegoException {
		synchronized (this) {
			final PredictiveAdjustment adjustment = doGetRequest(
					"alms/" + session.getAlmSn() + "/predictive/useradjustment",
					PredictiveAdjustment.class);
	...
	public void setPredictiveAdjustment(final int adjust) throws IndegoException {
		synchronized (this) {
			...
			doPutRequest("alms/" + session.getAlmSn() + "/predictive/useradjustment", adjustment,
					null);
		}
	}
	...
	public boolean getPredictiveMoving() throws IndegoException {
		synchronized (this) {
			final PredictiveStatus status = doGetRequest(
					"alms/" + session.getAlmSn() + "/predictive", PredictiveStatus.class);
	...
	public Date getPredictiveNextCutting() throws IndegoException {
		synchronized (this) {
			final PredictiveCuttingTime nextCutting = doGetRequest(
					"alms/" + session.getAlmSn() + "/predictive/nextcutting",
					PredictiveCuttingTime.class);
			return nextCutting.getNextCuttingAsDate();
		}
	}

	public DeviceCalendar getPredictiveExclusionTime() throws IndegoException {
		final DeviceCalendar calendar = doGetRequest(
				"alms/" + session.getAlmSn() + "/predictive/calendar", DeviceCalendar.class);
		return calendar;
	}
	 ...
	 * @throws IndegoInvalidCommandException
	 *             if the command was not processed correctly
	 * @throws IndegoException
	 *             in case of any unexpected event
	 */
	public void sendCommand(final DeviceCommand command_)
			throws IndegoInvalidCommandException, IndegoException {
		lastStateQueryTs = 0;
		final SetStateRequest request = new SetStateRequest();
	...
	/**
	 * This sends an authentication request to the server and unmarshals the
	 * result.
	 * 
	 * @return the result of the authentication request, when the authentication
	 *         was successfull.
	 * @throws IndegoAuthenticationException
	 *             in case of wrong authentication informations
	 * @throws IndegoException
	...
	private AuthenticationResponse doAuthenticate()
		...
		try {
			final HttpPost httpPost = new HttpPost(baseUrl + "authenticate");
			...
			final String json = mapper.writeValueAsString(authRequest);
			...
			final String responseContents = EntityUtils.toString(response.getEntity());
			final AuthenticationResponse authResponse = mapper.readValue(responseContents,
	 ...
	 * 
	 * @param urlSuffix
	 *            the path, to which the request should be sent
	 * @param returnType
	 *            the class to which the JSON result should be mapped; if null,
	 *            no mapping is tried and null is returned.
	 * @return the mapped result of the request
	 * @throws IndegoException
	 *             in case of any unexpected event
	 */
	private <T> T doGetRequest(final String urlSuffix, final Class<? extends T> returnType)
			throws IndegoException {
		try {
			final HttpGet httpRequest = new HttpGet(baseUrl + urlSuffix);
			...
			if (returnType == null) {
				...
				final T result = mapper.readValue(responseContents, returnType);
				return result;
			}
		} catch (final IOException ex) {
			throw new IndegoException(ex);
	 ...
	 *            the path, to which the request should be sent
	 * @param request
	 *            the data, which should be sent to the server (mapped to JSON)
	 * @param returnType
	 *            the class to which the JSON result should be mapped; if null,
	 *            no mapping is tried and null is returned.
	 * @return the mapped result of the request
	 ...
	 *             in case of any unexpected event
	 */
	private <T> T doPutRequest(final String urlSuffix, final Object request,
			final Class<? extends T> returnType) throws IndegoException {
		try {
			final HttpPut httpRequest = new HttpPut(baseUrl + urlSuffix);
			httpRequest.setHeader("x-im-context-id", session.getContextId());
